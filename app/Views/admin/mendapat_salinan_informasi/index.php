                ...
                <div class="row">
                    <div class="col-12">
                        <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                            <h4 class="mb-sm-0 font-size-18">Data Mendapat Salinan Informasi</h4>

                            <div class="page-title-right">
                                <ol class="breadcrumb m-0">
                                    <li class="breadcrumb-item"><a href="javascript: void(0);">Mendapat Salinan Informasi</a></li>
                                    <li class="breadcrumb-item active">Data Mendapat Salinan Informasi</li>
                                </ol>
                            </div>
                        ...
                        <div class="card">
                            <div class="card-body">
                                ...
                                <table id="example1" class="table table-bordered dt-responsive nowrap w-100">
                                            ...
                                            <div class="modal-dialog">
                                                <div class="modal-content">
                                                    <div class="modal-header">
                                                        <h5 class="modal-title" id="exampleModalLabel">Tambah Data Mendapat Salinan Informasi</h5>
                                                    ...
                                                    <div class="modal-body">
                                                        <form action="/admin/mendapat_salinan_informasi/save" method="POST">
                                                            <?= csrf_field(); ?>
                                                            <div class="mb-3">
                                    ...
                                    <tbody>
                                        <?php $i = 1; ?>
                                        <?php foreach ($tb_mendapat_salinan_informasi as $row) : ?>
                                            <tr>
                                                <td data-field="id_mendapat_salinan_informasi" style="width: 80px" scope="row"><?= $i++; ?></td>
                                                <td data-field="deskripsi"><?= $row['deskripsi']; ?>

                                                </td>
                                                <td style="width: 150px">
                                                    <button type="button" class="btn btn-warning btn-sm edit waves-effect waves-light" data-id="<?= $row['id_mendapat_salinan_informasi']; ?>" title="Edit">
                                                        <i class="fas fa-pencil-alt"></i> <span class="btn-text">Edit</span>
                                                    </button>
                                                    <button type="button" class="btn btn-danger btn-sm waves-effect waves-light sa-warning" data-id="<?= $row['id_mendapat_salinan_informasi']; ?>">
                                                        <i class="fas fa-trash-alt"></i> <span class="btn-text">Delete</span>
                                                    </button>
                                                    <button type="button" class="btn btn-success btn-sm save waves-effect waves-light d-none" data-id="<?= $row['id_mendapat_salinan_informasi']; ?>">
                                                        <i class="fas fa-save"></i> <span class="btn-text">Simpan</span>
                                                    </button>
                                                    <button type="button" class="btn btn-secondary btn-sm cancel waves-effect waves-light d-none" data-id="<?= $row['id_mendapat_salinan_informasi']; ?>">
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('click', '.edit', function() {
                ...
                var id_mendapat_salinan_informasi = $(this).data('id');
                var deskripsi = tr.find("[data-field='deskripsi']").text().trim(); // Hapus spasi ekstra

                // Simpan data awal sebelum diubah
                originalData[id_mendapat_salinan_informasi] = deskripsi;

                // Tambahkan id_mendapat_salinan_informasi ke dalam array editedIds
                if (!editedIds.includes(id_mendapat_salinan_informasi)) {
                    editedIds.push(id_mendapat_salinan_informasi);
                }
            ...
            $(document).on('click', '.cancel', function() {
                ...
                tr.find("[data-field='deskripsi']").text(originalData[id_mendapat_salinan_informasi]);

                // Hapus id_mendapat_salinan_informasi dari array editedIds
                editedIds = editedIds.filter(function(item) {
                    return item !== id_mendapat_salinan_informasi;
                });
            ...
            $(document).on('click', '.save', function() {
                ...
                // Loop melalui setiap id_mendapat_salinan_informasi yang sedang diedit
                editedIds.forEach(function(id_mendapat_salinan_informasi) {
                    var tr = $('button.edit[data-id="' + id_mendapat_salinan_informasi + '"]').closest('tr');
                ...
                $.ajax({
                    url: '<?= site_url('admin/mendapat_salinan_informasi/simpan_perubahan') ?>',
                    method: 'POST',
                    data: {
            ...
            $('.sa-warning').click(function(e) {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            type: "POST",
                            url: "<?= site_url('/admin/mendapat_salinan_informasi/delete') ?>",
                            data: {
                                id_mendapat_salinan_informasi: id_mendapat_salinan_informasi
                            },
                            dataType: 'json',
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#tambahButton').click(function() {
                ...
                $.ajax({
                    type: "POST",
                    url: "<?php echo site_url('/admin/mendapat_salinan_informasi/save'); ?>",
                    data: {
