                ...
                <div class="row">
                    <div class="col-12">
                        <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                            <h4 class="mb-sm-0 font-size-18">Data Alasan</h4>

                            <div class="page-title-right">
                                <ol class="breadcrumb m-0">
                                    <li class="breadcrumb-item"><a href="javascript: void(0);">Alasan</a></li>
                                    <li class="breadcrumb-item active">Data Alasan</li>
                                </ol>
                            </div>
                        ...
                        <div class="card">
                            <div class="card-body">
                                ...
                                <table id="example1" class="table table-bordered dt-responsive nowrap w-100">
                                            ...
                                            <div class="modal-dialog">
                                                <div class="modal-content">
                                                    <div class="modal-header">
                                                        <h5 class="modal-title" id="exampleModalLabel">Tambah Data Alasan</h5>
                                                    ...
                                                    <div class="modal-body">
                                                        <form action="/admin/alasan/save" method="POST">
                                                            <?= csrf_field(); ?>
                                                            <div class="mb-3">
                                                                <label for="deskripsi" class="col-form-label">Deskripsi :</label>
                                                                <textarea class="form-control" id="deskripsi" name="deskripsi" cols="30" rows="10" style="background-color: white;" placeholder="Masukkan Deskripsi" autofocus></textarea>
                                    ...
                                    <tbody>
                                        <?php $i = 1; ?>
                                        <?php foreach ($tb_alasan as $row) : ?>
                                            <tr>
                                                <td data-field="id_alasan" style="width: 80px" scope="row"><?= $i++; ?></td>
                                                <td data-field="deskripsi"><?= $row['deskripsi']; ?>

                                                </td>
                                                <td style="width: 150px">
                                                    <button type="button" class="btn btn-warning btn-sm edit waves-effect waves-light" data-id="<?= $row['id_alasan']; ?>" title="Edit">
                                                        <i class="fas fa-pencil-alt"></i> <span class="btn-text">Edit</span>
                                                    </button>
                                                    <button type="button" class="btn btn-danger btn-sm waves-effect waves-light sa-warning" data-id="<?= $row['id_alasan']; ?>">
                                                        <i class="fas fa-trash-alt"></i> <span class="btn-text">Delete</span>
                                                    </button>
                                                    <button type="button" class="btn btn-success btn-sm save waves-effect waves-light d-none" data-id="<?= $row['id_alasan']; ?>">
                                                        <i class="fas fa-save"></i> <span class="btn-text">Simpan</span>
                                                    </button>
                                                    <button type="button" class="btn btn-secondary btn-sm cancel waves-effect waves-light d-none" data-id="<?= $row['id_alasan']; ?>">
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('click', '.cancel', function() {
                ...
                tr.find("[data-field='deskripsi']").text(originalData[id_alasan]);

                // Hapus id_lembaga dari array editedIds
                editedIds = editedIds.filter(function(item) {
                    return item !== id_alasan;
                });
            ...
            $(document).on('click', '.save', function() {
                ...
                editedIds.forEach(function(id_alasan) {
                    var tr = $('button.edit[data-id="' + id_alasan + '"]').closest('tr');
                ...
                $.ajax({
                    url: '<?= site_url('admin/alasan/simpan_perubahan') ?>',
                    method: 'POST',
                    data: {
            ...
            $('.sa-warning').click(function(e) {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            type: "POST",
                            url: "<?= site_url('/admin/alasan/delete') ?>",
                            data: {
                                id_alasan: id_alasan
                            },
                            dataType: 'json',
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#tambahButton').click(function() {
                ...
                $.ajax({
                    type: "POST",
                    url: "<?php echo site_url('/admin/alasan/save'); ?>",
                    data: {
